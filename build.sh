...
for ARCH in arm64 x86_64; do
    ...
    "$SWIFTC" "${FLAGS[@]}" -target "${ARCH}-apple-macosx13.0" -O -parse-as-library \
        "$ROOT/Sources/QRCode.swift" "$ROOT/Sources/App.swift" -o "$BUILD/QuickQR-$ARCH"
done
...
"$SWIFTC" "${FLAGS[@]}" -target "$(uname -m)-apple-macosx13.0" "$ROOT/scripts/MakeIcon.swift" -o "$BUILD/MakeIcon"
"$BUILD/MakeIcon" "$BUILD/AppIcon.iconset"
cp "$BUILD/AppIcon.iconset/AppIcon.icns" "$APP/Contents/Resources/AppIcon.icns"
